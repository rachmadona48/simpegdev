...
//        var_dump($msg);exit;
    ...
    public function generateForm_sk_cuti(){
...
//        var_dump($msg);exit;
        ...
        switch ($form) {
            case 'jabatan_hist':
                ...
                if($action != null && $action == 'tambah'){
                    ...
                    if($this->user['user_group'] =='30' || $this->user['user_group'] =='10')
                    {
                        $data['listKolok']  = $this->infopegawai->getKolokAktifJoinPensiunmati($kolpeg1);
                        $data['listKlogad'] = $this->infopegawai->getKolokAktifJoinPensiunmati($klopeg1);
            ...
            case 'jabatanf_hist':
                ...
                if($action != null && $action == 'update'){                    
                    ...
                    $tmtpensiun = $data['infoJabatan']->TMTPENSIUN;
                    $jenis_sk = $data['infoJabatan']->JENIS_SK;
                    $tgakhir = $data['infoJabatan']->TGAKHIR;
                    $cekAktif = $this->infopegawai->cekAktifKolok($kolok);
                    $resaktif = $cekAktif->AKTIF;
            ...
            case 'pangkat':
                ...
                if($action == 'tambah')
                {
                    ...
                    if($this->user['user_group'] =='30' || $this->user['user_group'] =='10')
                    {
                        $data['listJenrub'] = $this->infopegawai->getMasterJenrubPangkatPengabdian($jenrub);
                    }
                    else if($this->user['user_group'] =='32')
                    {
                        $data['listJenrub'] = $this->infopegawai->getMasterJenrubPangkatPenerimaanDagun($jenrub);   
                    }
                    else
                    {
            ...
            case 'cuton_sk':                
                ...
                $data['id_hist'] = $this->input->post('key1');
                $data['jencuti'] = $this->input->post('key2');
                $data['action'] = $action;

                // echo $action;exit();
                
                if($action != null && ($action == 'sk_disetujui' || $action == 'sk_ditangguhkan')){                    
                    $data['infoCuti'] = $this->infopegawai->getCutiHist_cuton($nrk2);  
                    $pejtt = $data['infoCuti']->PEJTT;
                    $jencuti = $data['infoCuti']->JENCUTI;
                    $data['jenis_cuti'] = $data['infoCuti']->KETERANGAN;
                    $jenis_sk = $data['infoCuti']->JENIS_SK;
                    $data['listjensk'] = $this->infopegawai->getJenSK($jenis_sk);
                }else{
                    $data['listjensk'] = $this->infopegawai->getJenSKAdd();
            ...
            case 'rekap_cuti':
                ...
                if($action != null && $action == 'update'){ 
                    $data['data_rekap_cuti'] = $this->infopegawai->get_rekap_cuti_hist($nrk,$tahun);  
                }              
                break;
    ...
    public function hapusHist(){
        ...
        switch ($dest) {
            ...
            case 'skp':
                if ($action == 'hapus_flag') {
                    $return = $this->home->delete_flag_skp($key1,$key2);
                } else if ($action == 'hapus'){
...
// Comment out this line to see the text and debug such a problem.
    ...
    public function stat_riw_jbtn()
    {
        $NRK            = $this->uri->segment(3,0);
        $KOLOK          = $this->uri->segment(4,0);
        $KOJAB          = $this->uri->segment(5,0);
        $FILE_UPLOAD    = $this->uri->segment(6,0);
        $STATUS         = $this->uri->segment(7,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_JABATAN_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND KOLOK = '".$KOLOK."' AND KOJAB = '".$KOJAB."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Jabatan Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_JABATAN_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND KOLOK = '".$KOLOK."' AND KOJAB = '".$KOJAB."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Jabatan Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_pendidikan_formal()
    {
        $NRK            = $this->uri->segment(3,0);
        $JENDIK          = $this->uri->segment(4,0);
        $KODIK          = $this->uri->segment(5,0);
        $FILE_UPLOAD    = $this->uri->segment(6,0);
        $STATUS         = $this->uri->segment(7,0);
        $TGIJAZAH         = $this->uri->segment(8,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_PENDIDIKAN SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND JENDIK = '".$JENDIK."' AND KODIK = '".$KODIK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1' AND TGIJAZAH = TO_DATE('".$TGIJAZAH."', 'DD-MM-YYYY')"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Pendidikan Formal Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_PENDIDIKAN SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND JENDIK = '".$JENDIK."' AND KODIK = '".$KODIK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1' AND TGIJAZAH = TO_DATE('".$TGIJAZAH."', 'DD-MM-YYYY')"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Pendidikan Formal Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_pendidikan_non_formal()
    {
        $NRK            = $this->uri->segment(3,0);
        $JENDIK         = $this->uri->segment(4,0);
        $KODIK          = $this->uri->segment(5,0);
        ...
        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_PENDIDIKAN SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND JENDIK = '".$JENDIK."' AND KODIK = '".$KODIK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1' AND TGIJAZAH = TO_DATE('".$TGIJAZAH."', 'DD-MM-YYYY')"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Pendidikan Non Formal Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_PENDIDIKAN SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND JENDIK = '".$JENDIK."' AND KODIK = '".$KODIK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1' AND TGIJAZAH = TO_DATE('".$TGIJAZAH."', 'DD-MM-YYYY')"; 
            ...
            $pesan = 'File Upload Riwayat Pendidikan Non Formal Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_gapok()
    {
        $NRK            = $this->uri->segment(3,0);
        $TMT            = $this->uri->segment(4,0);
        $GAPOK          = $this->uri->segment(5,0);
        $FILE_UPLOAD    = $this->uri->segment(6,0);
        $STATUS         = $this->uri->segment(7,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_RB_GAPOK_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND GAPOK = '".$GAPOK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Gapok Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_RB_GAPOK_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND GAPOK = '".$GAPOK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Gapok Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_pangkat()
    {
        $NRK            = $this->uri->segment(3,0);
        $KOPANG            = $this->uri->segment(4,0);
        $GAPOK          = $this->uri->segment(5,0);
        $FILE_UPLOAD    = $this->uri->segment(6,0);
        $STATUS         = $this->uri->segment(7,0);
        ...
        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_PANGKAT_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND KOPANG = '".$KOPANG."' AND GAPOK = '".$GAPOK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Pangkat Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_PANGKAT_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND KOPANG = '".$KOPANG."' AND GAPOK = '".$GAPOK."'  AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat pangkat Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_skp()
    {
        $NRK            = $this->uri->segment(3,0);
        $TAHUN         = $this->uri->segment(4,0);
        $FILE_UPLOAD    = $this->uri->segment(5,0);
        $STATUS         = $this->uri->segment(6,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_SKP SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND TAHUN = '".$TAHUN."' AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Pangkat Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_SKP SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND TAHUN = '".$TAHUN."' AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat SKP Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_jbtn_func()
    {
        $NRK            = $this->uri->segment(3,0);
        $KOJAB          = $this->uri->segment(4,0);
        $KOLOK          = $this->uri->segment(5,0);
        $TMT            = $this->uri->segment(6,0);
        $FILE_UPLOAD    = $this->uri->segment(7,0);
        $STATUS         = $this->uri->segment(8,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_JABATANF_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND KOJAB = '".$KOJAB."' AND KOLOK = '".$KOLOK."' AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Jabatan Fungsional Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_JABATANF_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND KOJAB = '".$KOJAB."' AND KOLOK = '".$KOLOK."' AND FILE_UPLOAD = '".$FILE_UPLOAD."'  AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Jabatan Fungsional Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_cuti()
    {
        $NRK                = $this->uri->segment(3,0);
        $TMT                = $this->uri->segment(4,0);
        $JENCUTI            = $this->uri->segment(5,0);
        $TGAKHIR            = $this->uri->segment(6,0);
        $STATUS             = $this->uri->segment(7,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_CUTI_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."'  AND TMT = TO_DATE('".$TMT."', 'DD-MM-YYYY') AND JENCUTI = '".$JENCUTI."' AND TGAKHIR = TO_DATE('".$TGAKHIR."', 'DD-MM-YYYY') AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Cuti Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
            $sql = "UPDATE PERS_CUTI_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."'  AND TMT = TO_DATE('".$TMT."', 'DD-MM-YYYY') AND JENCUTI = '".$JENCUTI."' AND TGAKHIR = TO_DATE('".$TGAKHIR."', 'DD-MM-YYYY') AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Cuti Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_penghargaan()
    {
        $NRK                = $this->uri->segment(3,0);
        $KDHARGA            = $this->uri->segment(4,0);
        $STATUS             = $this->uri->segment(5,0);

        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_PENGHARGAAN SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND KDHARGA = '".$KDHARGA."' AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Penghargaan Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
           $sql = "UPDATE PERS_PENGHARGAAN SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND KDHARGA = '".$KDHARGA."' AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Penghargaan Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_hukdis()
    {
        $NRK                = $this->uri->segment(3,0);
        $TGSK               = $this->uri->segment(4,0);
        $STATUS             = $this->uri->segment(5,0);


        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_DISIPLIN_HIST SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND TGSK =  TO_DATE('".$TGSK."', 'DD-MM-YYYY') AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Hukuman Disiplin Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
           $sql = "UPDATE PERS_DISIPLIN_HIST SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND TGSK =  TO_DATE('".$TGSK."', 'DD-MM-YYYY') AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Hukuman Disiplin Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function stat_riw_hubkel()
    {
        $NRK                = $this->uri->segment(3,0);
        $HUBKEL             = $this->uri->segment(4,0);
        $STATUS             = $this->uri->segment(5,0);


        if($STATUS == 2)
        {
            $sql = "UPDATE PERS_KELUARGA SET STATUS_FILE_UPLOAD = '2' WHERE NRK = '".$NRK."' AND HUBKEL =  '".$HUBKEL."' AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);
            $pesan = 'File Upload Riwayat Hubungan Keluarga Berhasil Di Approve';
        }
        elseif($STATUS == 3)
        {
           $sql = "UPDATE PERS_KELUARGA SET STATUS_FILE_UPLOAD = '3' WHERE NRK = '".$NRK."' AND HUBKEL =  '".$HUBKEL."' AND STATUS_FILE_UPLOAD = '1'"; 
            $this->db->query($sql);

            $pesan = 'File Upload Riwayat Hubungan Keluarga Telah ditolak untuk dilakukan Revisi';
        }
        else
        {
            $pesan = 'Gagal Proses';
    ...
    public function reset_file()
    {
        ...
        $jns_riwayat        = $this->uri->segment(3,0);
        $nm_file            = $this->uri->segment(4,0);
        $nrk                = $this->uri->segment(5,0);
        
        if($jns_riwayat == 'jabatan')
        {
            $kolok          = $this->uri->segment(6,0);
            $kojab          = $this->uri->segment(7,0);
            $tmt            = $this->uri->segment(8,0);
            
            $data_lama     = '/'.$nrk.'/jabatan/'.$nm_file;
        }
        ...
        if($jns_riwayat == 'jabatan')
        {
            // hapus data di DB DOK ASN
            $sql = "DELETE FROM \"tbl_hist_pegawai_jabatan\" WHERE \"nrk\" = '".$nrk."' AND \"kolok\" = '".$kolok."' AND \"kojab\" = '".$kojab."' AND \"file\" = '".$nm_file."'"; 

            $this->db2->query($sql);

            $Quer = "UPDATE PERS_JABATAN_HIST set FILE_UPLOAD = '' null, STATUS_FILE_UPLOAD = '' WHERE NRK = '$nrk' AND TMT =  TO_DATE('".$tmt."', 'DD-MM-YYYY') AND KOLOK = '$kolok'  AND KOJAB = '$kojab'";
            $this->db->query($Quer);
            
            $pesan = 'Riwayat Jabatan Struktural berhasil direset';
        }
