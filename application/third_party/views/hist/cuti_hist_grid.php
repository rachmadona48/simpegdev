...
<div class="wrapper wrapper-content animated fadeInRight">
	<div class="row">
        <div class="col-lg-12">
            <div class="contact-box">
            	<div>
					 <button class="btn btn-success pull-right" onclick="add_cutihist()"><i class="glyphicon glyphicon-plus"></i> Tambah Cuti History</button>
				</div>
            	<div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="modal-title">Form Cuti History</h3>
      </div>
      <div class="modal-body form">
        <form action="#" id="form" class="form-horizontal">
          ...
          <div class="form-body">
            ...
            <div class="form-group">
              <label class="control-label col-md-3">JENCUTI</label>
              <div class="input-group col-sm-4">
                 <!--<input type="text" id="jencuti" name="jencuti" placeholder="JENCUTI" value="<?php echo isset($JENCUTI) ? $JENCUTI : ""; ?>" class="form-control">-->
              
                  <select class="" name="jencuti" id="jencuti" tabindex="2" placeholder="JENCUTI">
                    <option></option>
                  </select>
              </div>
            </div>
		...
		<script type="text/javascript" language="javascript" >
          ...
          $('#data_2 .input-group.date').datepicker({
           ...
           $('#data_3 .input-group.date').datepicker({
            ...
            $(".select2_jencuti").select2({
                  placeholder: "Pilih Kode Jenis Cuti"                    
              }); 
            $(".select2_pejtt").select2({
				...
				var dataTable = $('#employee-grid').DataTable( {
					...
					"ajax":{
						url :"<?php echo base_url(); ?>index.php/hist/cuti_hist/data", // json datasource
	...
	function add_cutihist()
  	{
       $.ajax({
          url: "<?php echo base_url(); ?>index.php/hist/Cuti_hist/getJencuti",
          type: "post",
          dataType: 'json',
          beforeSend: function() {                                                        
              $('#jencuti').hide();
      ...
      $.ajax({
          url: "<?php echo base_url(); ?>index.php/hist/Cuti_hist/getPejtt",
          type: "post",
          dataType: 'json',
          beforeSend: function() {                                                        
              $('#pejtt').hide();
      ...
      $('.modal-title').text('Tambah Cuti History'); // Set Title to Bootstrap modal title
  	}
  ...
  function save()
  {
      ...
      if(save_method == 'add') 
      {
          url = "<?php echo site_url('hist/cuti_hist/ajax_add')?>";
      }
      else
      {
        url = "<?php echo site_url('hist/cuti_hist/ajax_update')?>";
      }
  ...
  function edit_cutihist(id1,id2)
  {
      save_method = 'update';
      $('#form')[0].reset(); // reset form on modals
      ...
      $.ajax({
        url : "<?php echo site_url('hist/cuti_hist/ajax_edit/')?>/"+id1+'/'+id2,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="nrk"]').val(data.NRK);
            $('[name="tmt"]').val(data.conv);
            //$('[name="tmt"]').val(data.TMT);
            $('[name="jencuti"]').val(data.JENCUTI);
            $('[name="tgakhir"]').val(data.TGAKHIR);
            $('[name="nosk"]').val(data.NOSK);
            $('[name="tgsk"]').val(data.TGSK);
            $('[name="pejtt"]').val(data.PEJTT);            
            
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Cuti History'); // Set title to Bootstrap modal title
  ...
  function delete_cutihist(id1,id2)
  {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data to database
          $.ajax({
            url : "<?php echo site_url('hist/cuti_hist/ajax_delete')?>/"+id1+'/'+id2,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
