                            ...
                            <div class="row">
                                ...
                                <form class="form-horizontal" id="defaultForm" method="post">
                                    ...
                                    <div class="form-group">
                                        <div class="col-md-12">
                                            <div class="col-md-3">
                                                <label for="no_surat_bu">No Surat Biro Umum</label>
                                            </div>
                                            <div class="col-md-9">
                                                <input class="form-control" type="text" name="no_surat_bu" id="no_surat_bu" >
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
...
<?php } ?>
        ...
        $(document).ready(function(){
            ...
            $('#btn_forward').click(function(e){
                ...
                var no_surat_bu = $('#no_surat_bu').val();
                // console.log()
                if(tgl_surat !="" && no_surat_bu !="")
                {
                    insert_data();
                    alert ('Data Berhasil Diteruskan');
                    window.location.href = 'http://simpegdev.jakarta.go.id/biroumum/';    
                }
                else
                {
        ...
        function insert_data(){
            ...
            var jns_prm = $('#jenis_permohonan').val();
            var no_surat_bu_post = $('#no_surat_bu').val();
            var tgl_surat_post = $('#tgl_surat').val();
            var id_trx_bh_post = [];
            $('input[name^=id_trx_bh]').each(function(){
                id_trx_bh_post.push($(this).val());
            });
            $.ajax({
                url: '<?php echo base_url("biroumum/insert")?>/',
                type: 'post',
                data: {
                    id_trx_bh: id_trx_bh_post, no_surat_bu: no_surat_bu_post, tgl_surat: tgl_surat_post, ref_prm: ref_prm, jns_prm:jns_prm
                },
                dataType: 'json',
                success: function(data){
                    console.log(data);
        ...
        {
            var id_trx_bh_post = [];
            $('input[name^=id_trx_bh_post]').each(function(){
                id_trx_bh_post.push($(this).val());
            });
            
            $.ajax({
                url: '<?php echo base_url("biroumum/update")?>/',
                type: 'post',
                data: {
                    id_trx_bh: id_trx_bh_post
                },
                dataType: 'json',
                success: function(data){
                    console.log(data);
        ...
        function get_detail_pegawai(nrk_post){
            ...
            //var jns_prm = ('#jenis_permohonan').val();
            $.ajax({
                url: '<?php echo base_url("biroumum/get_detail_pegawai")?>/',
                type: 'post',
                data: {
                    nrk: nrk_post
                },
                ...
                success: function(data){
                    ...
                    var ref_prm = $('#ref_permohonan').val();
                    var jns_prm = $('#jenis_permohonan').val();
                    persyaratan(data.nrk, ref_prm, jns_prm, data.id_trx_bh);
                }
            })
        }
        ...
        function persyaratan(nrk_post, id_ref_prm,id_jns_prm, id_trx){
            ...
            $.ajax({
                url: '<?php echo base_url("biroumum/get_persyaratan"); ?>/',
                type: "post",
                data: {
                   NRK: nrk_post, ID_PERMOHONAN: id_ref_prm, ID_JENIS: id_jns_prm, ID_TRX_BH: id_trx
                },
                dataType: 'JSON',
                success: function(data) {
        ...
        function getdatapermohonan(jns_prm){
            ...
            var dtbl= $('#tbl-grid3').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    url: '<?php echo base_url("index.php/biroumum/get_data_permohonan"); ?>/',
                    type: "post",
                     data: {
