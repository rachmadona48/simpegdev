            ...
            <div class="ibox-content">
                ...
                <div class="row" >
                    ...
                    <tr class="gradeX existing" height="20" id="row-<?php echo $value->id?>" >
                        ...
                        <td width="10%"> 
                         <?php
                            ...
                            // foreach ($arr_select as $key => $values) {
                            //     # code...
                            //     if ($values==$value->output_realisasi){
                            //         echo '<option value="'.$value->output_realisasi.'" selected>'.$value->output_realisasi.'</option>';
                            //     }
                            //     else {
                    ...
                    <tr class="gradeY tambahan tambahan-existing-kreatifitas" height="20" id="row-<?php echo $value->id?>" >
                        <td height="10" class="center no"><?=$no?></td>
                        <td>
                            <?=$value->kegiatan?>
                        ...
                        <td align="right" width="6%">
                            <?php
                            // echo '<select class="form-control" id="waktu" name="waktu" disabled>';
                            // for ($x = 1; $x <= 12; $x++) {
                            //     if ($x==$value->waktushow) {
                            //      echo '<option value="'.$value->waktushow.'" selected>'.$value->waktushow.'</option>';
                            //     }
                            //     else {
                            //         echo '<option value="'.$x.'">'.$x.'</option>';
                            //     }
                    ...
                    <div class="row">                            
                            ...
                            <button type='button' class='btn btn-primary btn-submit' disabled><i class='fa'></i>Kirim Tugas Tambahan</button>
                            <button type='button' class='btn btn-primary btn-tambah' onclick="fnClickAddRow_kreatifitas()">Tambah Kreatifitas</button>
                            <button type='button' class='btn btn-primary btn-submit' disabled><i class='fa'></i>Kirim Kreatifitas</button>
                            
                        </div>
                    <button type="button" class="btn btn-success btn-simpan">Setujui</button>
    ...
    <script type="text/javascript">
       $(document).ready(function() {            
            $(".btn-simpan").click(function(){                
                var arr_data = getArrTableExisting($(".gradeX"));
                var arr_add_existing_kreatifitas = getArrTableExisting($(".tambahan-existing-kreatifitas"));
                var arr_add_new_kreatifitas = getArrTableExisting($(".tambahan-new-kreatifitas"));
                // console.log(arr_data);
                console.log(arr_add_existing_kreatifitas);
                $.ajax({
                    type: "POST",
                    data: {data:arr_data, skp_id: '<?php echo $skp_id ?>',new_kreatifitas:arr_add_existing_kreatifitas},
                    url: "<?php echo base_url(); ?>index.php/skp/submit_pengukuran_realisasi",
...
//alert('tes');
   function fnClickAddRow(){
            ...
            var html = '<tr class="gradeX tambahan tambahan-new">';
                ...
                html = html + '<td><input type="text" attr-id-keg="" name="kegiatan[]" class="form-control" /></td>';
               ...
               var arr_select = ["Laporan","Dokumen","Surat","Berkas"];
                ...
                html = html + '<td><input type="text" name="ak_realisasi[]" class="form-control" onkeypress="allowNumbersOnly(event)"/></td>';
                html = html + '<td><input type="text" name="qty_realisasi[]" class="form-control" onkeypress="allowNumbersOnly(event)"/></td>';
                var arr_select = ["Laporan","Dokumen","Surat","Berkas"];
                var arr_select = [];
                    <?php foreach ($doc_type as $key => $value) { ?>
                        arr_select.push({key: "<?php echo $value->id; ?>", value: "<?php echo $value->name; ?>"});
                ...
                var str_html = onSelectOutput(arr_select, "");  
                html = html + '<td width="10%">' + str_html + '</td>';
                html = html + '<td><input type="text" name="quality_realisasi[]" class="form-control" onkeypress="allowNumbersOnly(event)"/></td>';
                html = html + '<td>';
                html = html + '<select class="form-control" id="time" name="time[]">';
                html = html + '<?php for ($x = 1; $x <= 12; $x++) { ?>';
                html = html + '<?php echo "<option value=".$x." selected>".$x."</option>"?>';
                ...
                html = html + '<td>bulan</td>'; 
                html = html + '<td><input type="text" name="kual[]" class="form-control" onkeypress="allowNumbersOnly(event)"/></td>';
                html = html + '<td align="right">268</td>';
                html = html + '<td align="right">88</td>';  
    ...
    function onDelete(elm){              
        ...
        function displayTable(data){
            ...
            $(data).each(function(i,obj){ 
             //alert('test');die;
                ...
                var strhtml = "";
                if (type_kegiatan==2){
                    strhtml = strhtml + '<tr class="gradeTambahan" id="row-' + obj.id + '">';
                }else{
                ...
                strhtml = strhtml + '<td>'+ doc_master[output_realisasi] +'</td>';
        ...
        function onChangeTypeKegiatan(el){
            ...
            if (val==""){
                ...
                // /// from here
                // var table = $('#tbl-popup-kegiatan').DataTable({
                //     "processing": true,
                //     "serverSide": true,
                //     "ajax": {
                //         "url" : "<?php echo base_url() ?>index.php/skp/list_popup_kegiatan1",
                //         "type": "POST",
                //         "data" : {
                //             "cmd" : "refresh",
                //             "from": "bvb",
                ...
                $.ajax({
                    ...
                    success: function(msg){                                                                                
                        var json = JSON.parse(msg);
                        if (json.response=="SUKSES"){                        
                            displayTableKegiatan(el,json.data);                                                    
                        }                                    
                    }
                });   
        ...
        function displayTableKegiatan(el,json){
            var str_html = "";
            ...
            var element = $(el).prev();
            m_element = {el:el, element:element, id: "", val: ""};
            $(".kegiatan-body tr").click(function() {  
                $(".kegiatan-body tr").css("background-color","#F5F5F6");
                $(this).css("background-color","red");
                var id = $(this).attr("attr-id");
                var val = $(this).attr("attr-value");                
                m_element = {el:el, element:element, id: id, val: val};
            });
