...
class Jasper {
    ...
    $this->cndriver=$cndriver;
        if($cndriver=="mysql") {
            ...
            if(!$this->con) {
                $this->myconn = oci_connect($db_user,$db_pass,$db_host);
                // $this->myconn = oci_connect('aset','aset','10.15.34.60/asetdev');
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        ...
        if($cndriver=="odbc") {
            $result=oci_parse( $this->myconn,$this->sql);
            oci_execute($result);
