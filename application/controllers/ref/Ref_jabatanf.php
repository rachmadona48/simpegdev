<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Ref_jabatanf extends CI_Controller {

	private $user=array();

	public function __construct()
   	{
    	parent::__construct();
    	$this->load->helper('url');
    	$this->load->library('session');
    	

    	if ($this->session->userdata('logged_in')) {            

            $session_data       = $this->session->userdata('logged_in');
            $this->user['id']     	= $session_data['id'];
            $this->user['username']  	= $session_data['username'];
        }else{
			redirect(base_url().'index.php/login', 'refresh');
		}	    
   	}

	public function index()
	{	
			$this->load->view('head/header', $this->user);
			$this->load->view('head/menu');
			$this->load->view('ref/ref_jabatanf_grid.php');
			$this->load->view('head/footer');		
	}	



	public function data()
	{		
		$requestData = $this->input->post();	

		$columns = array( 
		// datatable column index  => database column name
			0 => 'kojab',
			1 => 'najabs',
			2 => 'najabl',
			3 => 'tunfung',
			4 => 'job_class1',
			5 => 'job_class2',
			6 => 'point',
			7 => 'tunjab',
			8 => 'point_207',
			9 => 'peringkat',
			10 => 'tahap1',
			11 => 'tahap2',
			12 => 'user_id',
			13 => 'term',
			14 => 'tg_upd',			
		);

		// getting total number records without any search
		$sql = "SELECT rownum, x.* FROM (SELECT rownum as rn, kojab,najabs,najabl, tunfung, job_class1, job_class2, point, tunjab, point_207, peringkat, tahap1, tahap2, user_id, term, tg_upd";
		$sql .= " FROM pers_kojabf_tbl
				WHERE (0=0)) X";		
		
		$query= $this->db->query($sql);
		
		$data = array();		

		foreach($query->result() as $row){
			$nestedData=array(); 

			$nestedData[] = $row->ROWNUM;
			$nestedData[] = $row->KOJAB;
			$nestedData[] = $row->NAJABS;
			$nestedData[] = $row->NAJABL;
			$nestedData[] = $row->TUNFUNG;
			$nestedData[] = $row->JOB_CLASS1;
			$nestedData[] = $row->JOB_CLASS2;
			$nestedData[] = $row->POINT;
			$nestedData[] = $row->TUNJAB;
			$nestedData[] = $row->POINT_207;
			$nestedData[] = $row->PERINGKAT;
			$nestedData[] = $row->TAHAP1;
			$nestedData[] = $row->TAHAP2;
			$nestedData[] = $row->USER_ID;			
			$nestedData[] = $row->TERM;
			$nestedData[] = $row->TG_UPD;							
			
			$data[] = $nestedData;
			
		}	

		$json_data = array(
					"data" => $data   
					);

		echo json_encode($json_data);  
	}
}
