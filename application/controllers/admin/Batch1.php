  ...
  //       $bln= substr($tgl_batch, 5,2);
	...
	public function printTKD2()
	{
		...
		foreach ($printTKD2 as $key) {
			...
			$spmu 			= str_pad($key->SPMU, 5," ",STR_PAD_RIGHT);
			$klogad 		= str_pad($key->KLOGAD, 10," ",STR_PAD_RIGHT);
			$nalokl 		= str_pad($key->NALOKL, 50," ",STR_PAD_RIGHT);
			$njtundabersih 	= str_pad($key->NJTUNDABERSIH, 10," ",STR_PAD_RIGHT);
			$thbl 			= str_pad($key->THBL, 6," ",STR_PAD_RIGHT);
			$kdesel 		= str_pad($key->KDESEL, 5," ",STR_PAD_RIGHT);
	...
	public function printTransport()
	{
		...
		foreach ($printTrans as $key) {
			...
			$klogad 		= str_pad($key->KLOGAD, 10," ",STR_PAD_RIGHT);
			$nalokl 		= str_pad($key->NALOKL, 50," ",STR_PAD_RIGHT);
			$jumber 		= str_pad($key->JUMBER, 10," ",STR_PAD_RIGHT);
			$thbl 			= str_pad($key->THBL, 6," ",STR_PAD_RIGHT);
			$kdesel 		= str_pad($key->KDESEL, 5," ",STR_PAD_RIGHT);
	...
	public function printGAJI_ptt(){
		...
		foreach ($printGJ as $key) {
			$nama 		= str_pad($key->NAMA, 25," ",STR_PAD_RIGHT);
			...
			$naloks 	= str_pad($key->NALOKS, 50," ",STR_PAD_RIGHT);
			$gajibersih = str_pad($key->GAJIBERSIH, 8," ",STR_PAD_RIGHT);
	...
	public function print_TPP_PTT(){
		...
		foreach ($printGJ as $key) {
			...
			$klogad 	= str_pad($key->KLOGAD, 9," ",STR_PAD_RIGHT);
			$naloks 	= str_pad($key->NALOKS, 50," ",STR_PAD_RIGHT);
			$tppbersih  = str_pad($key->TPPBERSIH, 15," ",STR_PAD_RIGHT);
