<?php 
 ...
 class Model_skp extends CI_Model {
    ...
    //list pengukuran
    var $table = 'SKP';
    var $column_order = array('nrk','nama','thbl',null); //set column field database for datatable orderable
    var $column_search = array('nama','nrk','thbl'); //set column field database for datatable searchable just firstname , lastname , address are searchable
    ...
    {        
        ...
        $this->tblskpdetail = 'SKP_DETAILS';
        $this->tblskp = 'SKP';                        
        $this->skpdetail = 'id';    
        $this->tblskptarget = 'SKP_TARGET';  
        $this->skp = 'id';
        ...
        if($nrk != '000000'){
	        ...
	        if($q_cek_pimpinan->jml >= 1){
                ...
                if($q_cek_eselon->KOLOK == '000001102' && $q_cek_eselon->KOJAB == '000000'){
                    ...
                    $nrk_atasan = $cek_atasan_sekda->NRK;
                }else{
	            ...
	            if($q_cek_eselon->NESELON == 'NON ESELON'){
                    ...
                    $nrk_atasan = $q_cek_atasan2->NRK;
                    //echo 'nip - nama'.$nrk_atasan;exit();
                    //var_dump($q_cek_atasan2);
	                ...
	                if($q_struktur_kojab==NULL){
		                ...
		                if($q_sts_atasan->JML >= 1){
                            ...
                            //added by pemda, 25-4-2019
                             $nrk_atasan = $q_atasan->NRK;
		                }else{
...
// END DETAIL CUTI
   ...
   // ========================================= akses bkd kesra ==============================================
    ...
    function insert($nrk = "", $username ="", $nrk_atasan="", $id = "", $params = array()){                                             
            $len = count($params);            
            $arr_params = array();            
            if ($len>0){                                        
                foreach ($params as $key => $value) {
                    ...
                    $arr_fields = ["id"];
                    $arr_values = [$id];
                    $skp = $this->get_data($arr_fields, $arr_values, "SKP"); 
                    $status_approvement = 0;
                    if (count($skp)>0){
                        $status_approvement = $skp[0]->status_approvement;
                    }

                    $version = $this->max_skp_details($id);
                    ...
                    if ($status_approvement == DIKIRIM){
                        $fields["status_koreksi"] = DIKOREKSI;
                    }else{
                        $fields["status_koreksi"] = TIDAKKOREKSI;
                    }
                    $sql = $this->sql_insert2($this->tblskpdetail, $fields);
                };
            }       
        }

        function update($id= "", $value = array(), $nrk = "", $type = ""){   
            $arr_column = array(
                "kegiatan" => $value[0],
                "ak"=> $value[1]);
            $arr_condition = array($this->skpdetail => $id);            
            $this->insert_log($this->tblskpdetail, $id);            
            $arr_column = array(
                "kegiatan"=> $value[0],
                "id_kegiatan"=> $value[1],
                "jenis_kegiatan"=> $value[2],
            ...
            if ($type!=""){
                if ($type=="approval"){
                    $arr_column["status_koreksi"] = DIKOREKSI;
                }
            }
            // print_r($arr_column);die();
            $arr_condition = array("id" => $id);

            $this->sql_update($arr_column, $arr_condition, "SKP_DETAILS");            
        }
        
        function get_data($arr_fields = array(), $arr_value = array(), $table_name = "", $select = ""){    
            foreach ($arr_fields as $key => $value) {                                
                if ($value=='deleted_at'){                                        
                    $this->db->where($value, null);
                }else{
                    $this->db->where($value , $arr_value[$key]);    
                }                
            }                                   
            if ($select!=""){
        ...
        function get_skp_kegiatan_tupoksi($kolok = "", $kojab = ""){  
            if ($kolok!=""){
                $this->db->where("kolok",$kolok);
            }
            if ($kojab!=""){
                $this->db->where("kojab",$kojab);
            }
            $this->db->order_by('no');
            $query = $this->db->get("SKP_TUPOKSI");
            return $query->result();
        }

        public function get_skp_target($id, $version = ""){            
            $this->db->select('SKP_TARGET.*,
                                SKP_DETAILS.*,
                                SKP_MASTER_DOC_TYPE.name as outputshow,                                
                                SKP_TARGET.quantity quantityshow, 
                                SKP_TARGET.quality qualityshow,
                                SKP_TARGET.waktu waktushow, 
                                SKP_TARGET.total_month total_monthshow,  
                                SKP_TARGET.biaya biayashow, 
                                SKP_TARGET.status statusshow');
            $this->db->join('SKP_TARGET', 'SKP_TARGET.skp_detail_id = SKP_DETAILS.id');
            $this->db->join('SKP_MASTER_DOC_TYPE', 'SKP_TARGET.output = SKP_MASTER_DOC_TYPE.id', 'left');
            $this->db->where('SKP_DETAILS.skp_id', $id);            
            $this->db->where('SKP_TARGET.deleted_at', null);            
            $this->db->where('SKP_DETAILS.deleted_at', null);            
            if ($version!==""){
                $this->db->where('SKP_TARGET.version', $version);
            }else{
                $this->db->where('SKP_TARGET.status', '1');    
            }


            $this->db->order_by('SKP_DETAILS.created_at');
            $this->db->from('SKP_TARGET');
            $query = $this->db->get();            
            return $query->result();
        }
        public function get_skp_target_pengukuran($id, $version = ""){
            $this->db->select('SKP_DETAILS.*,
                               SKP_MASTER_DOC_TYPE.name as outputshow,                                
                               SKP_DETAILS.quantity quantityshow, 
                               SKP_DETAILS.quality qualityshow,
                               SKP_DETAILS.waktu waktushow, 
                               SKP_DETAILS.total_month total_monthshow,  
                               SKP_DETAILS.biaya biayashow, 
                               SKP_DETAILS.status statusshow');            
            $this->db->join('SKP_MASTER_DOC_TYPE', 'SKP_DETAILS.output = SKP_MASTER_DOC_TYPE.id', 'left');            
            $this->db->where('SKP_DETAILS.skp_id', $id);                        
            $this->db->where('SKP_DETAILS.deleted_at', null);            
            if ($version!==""){
                $this->db->where('SKP_DETAILS.version', $version);
            }else{
                $this->db->where('SKP_DETAILS.status', '1');    
            }
            $this->db->order_by('SKP_DETAILS.created_at');
            $this->db->from('SKP_DETAILS');
            $query = $this->db->get();            
            return $query->result();
        }

        public function get_skp_new_target($id, $version = ""){            
            $this->db->select('SKP_DETAILS.*,
                               SKP_MASTER_DOC_TYPE.name as outputshow,                                
                               SKP_DETAILS.quantity quantityshow, 
                               SKP_DETAILS.quality qualityshow,
        ...
        public function max_skp_target($id){
            $this->db->select('max("version") version');
            $this->db->where('SKP_TARGET.skp_id', $id);
            $this->db->where('SKP_TARGET.status', '1');
            $this->db->where('SKP_TARGET.deleted_at', null);
            $this->db->from('SKP_TARGET');
            $query = $this->db->get();            
         ...
         public function max_skp_details($id){
            $this->db->select('max("version") version');
        ...
        public function get_total($str_order = "", $str_where = ""){
    ...
    public function get_total_skp_join_penilaian_keputusan_atasan_pejabat_penilai($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select count(*) as total from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_KEBERATAN.' AND   SKP_PENILAIAN."nrk_atasan_pejabat"=\''.$nrk.'\'';        
        return $this->db->query($query)->row();
    }

    public function get_total_skp_join_penilaian($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select count(*) as total from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_DIKIRIM;        
        return $this->db->query($query)->row();
    }

     public function get_total_penilaian_bawahan($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select count(*) as total from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND  "SKP"."status_approvement">= '.DIKIRIM.' AND "SKP"."atasan"=\''.$nrk.'\'';        
        return $this->db->query($query)->row();    
    }

    public function get_all_page_total($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $query = 'select count(*) as total from "'.$this->table.'" '.$str_where.' '.$str_order;
        return $this->db->query($query)->row();
    }

    public function get_all_page($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $query = 'select * from "'.$this->table.'" '.$str_where.' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';
        return $this->db->query($query)->result();
    }



    public function get_all_page_approval($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        if ($str_where!=""){
            $str_where = $str_where.' AND "status_approvement">=\''.DIKIRIM.'\'';
        }
        $query = 'select * from "'.$this->table.'" '.$str_where.'  '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';
        return $this->db->query($query)->result();
    }

    public function get_skp_join_penilaian($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_DIKIRIM.' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    public function get_skp_join_penilaian_bawahan($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND  "SKP"."status_approvement">= '.DIKIRIM.' AND "SKP"."atasan"=\''.$nrk.'\' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    public function get_skp_join_penilaian_keputusan_atasan_pejabat_penilai($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_KEBERATAN.' AND   SKP_PENILAIAN."nrk_atasan_pejabat"=\''.$nrk.'\' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    public function get_skp_join_hasilpenilaian($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND SKP."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_DIKIRIM.' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    // added by pemda, 13-6-19
     public function get_skp_join_perilaku($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PERILAKU."perilaku_approvement",skp.* from '.$this->table.' left JOIN SKP_PERILAKU on SKP."id"=SKP_PERILAKU."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PERILAKU."perilaku_approvement">= '.PENILAIAN_DIKIRIM.' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    public function get_skp_join_hasil_perilaku($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PERILAKU."perilaku_approvement",skp.* from '.$this->table.' left JOIN SKP_PERILAKU on SKP."id"=SKP_PERILAKU."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.MUTASI.' AND  SKP_PERILAKU."perilaku_approvement">= '.PERILAKU_DONE.' AND "nrk_atasan_baru"=\''.$nrk.'\' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }

    public function get_total_skp_join_hasil_perilaku($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select count(*) as total from '.$this->table.' left JOIN SKP_PERILAKU on SKP."id"=SKP_PERILAKU."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PERILAKU."perilaku_approvement">= '.PERILAKU_DONE.' AND "nrk_atasan_baru"=\''.$nrk.'\'';                
        return $this->db->query($query)->row();
    }

    //get_total_skp_join_hasil_perilaku
    public function get_count_list_perilaku_bawahan_mutasi($str_order = "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $count='select count(*) as total from ( select "nrk" from SKP where "atasan"='.$nrk.' and "deleted_at" is null group by "nrk") tblskp';         
        return $this->db->query($count)->row();
    }

    public function get_skp_join_perilaku_bawahan($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $count='select count(*) as total from ( select "nrk" from SKP where "atasan"='.$nrk.' and "deleted_at" is null group by "nrk") tblskp'; 

        $row = $this->db->query($count)->row();
        $row = $row->TOTAL;
        $query='SELECT * 
                FROM
                    skp
                    ...
                    WHERE
                        kolok IN (
                            ...
                            FROM
                                PERS_JABATAN_HIST 
                            WHERE
                                NRK IN ( SELECT "nrk" FROM SKP WHERE "atasan" = '.$nrk.' AND "deleted_at" IS NULL GROUP BY "nrk" ) 
                                ORDER BY
                                TG_UPD DESC OFFSET 0 ROWS FETCH NEXT '.$row.' ROWS ONLY 
                                ) 
                            AND kojab IN (
                                SELECT
                                    kojab 
                            ...
                            AND NRK IN ( SELECT "nrk" FROM SKP WHERE "atasan" = '.$nrk.' AND "deleted_at" IS NULL GROUP BY "nrk" ) 
                            ORDER BY
                                tg_upd ASC OFFSET 0 ROWS FETCH NEXT '.$row.' ROWS ONLY 
                                ) tbl_latest ON skp."nrk" = tbl_latest.nrk 
                            AND tbl_latest.TG_UPD >= SKP."startdate" 
                            AND tbl_latest.TG_UPD <= SKP."enddate" 
                            AND SKP."status_approvement" >=5'; 


        return $this->db->query($query)->result();
    }
    // added by pemda, 20-6-2019
    public function get_skp_mutasi($skp_id){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        // $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND  "SKP"."status_approvement">= '.SKP_DITERIMA.' AND "SKP"."atasan"=\''.$nrk.'\' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';

        // $count='select count(*) as total from ( select "nrk" from SKP where "atasan"=\'164901\' and "deleted_at" is null group by "nrk") tblskp'; 

        // $row = $this->db->query($count)->row();
        // $row = $row->TOTAL;
        // $query_mutasi_all_bawahan=$this->get_mutasi_bawahan('164901', $row); 
        
        // $result1 = $this->db->query($query_mutasi_all_bawahan)->result();

        $query_mutasi_pegawai=$this->get_mutasi_by_nrk($nrk, $skp_id);         
        $result = $this->db->query($query_mutasi_pegawai)->row();
    ...
    public function get_mutasi_bawahan($nrk, $row){
        $query='SELECT * 
                FROM
                    skp
                INNER JOIN (
    ...
    public function get_mutasi_by_nrk($nrk, $skp_id = ""){
        $query = 'SELECT * from skp  inner join 
            (SELECT
                TG_UPD,
                TMT,
                nrk 
            ...
            WHERE
                NRK IN (\''.$nrk.'\') 
            ORDER BY
                TG_UPD DESC OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY 
                ) 
                AND kojab IN (
            SELECT
                kojab 
            ...
            ORDER BY
                ...
                AND NRK IN (\''.$nrk.'\') 
            ORDER BY
                tg_upd ASC OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY
                ) tbl_latest on skp."nrk"=tbl_latest.nrk 
            AND tbl_latest.TG_UPD >= SKP."startdate"
            AND tbl_latest.TG_UPD <= SKP."enddate"
            AND SKP."status_approvement">=5';
            if ($skp_id!=''){
                $query = $query.'AND SKP."id"=\''.$skp_id.'\'';    
            }
    ...
    public function get_skp_join_perilaku_keputusan_atasan_pejabat_penilai($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $session_data = $this->session->userdata('logged_in');
        $nrk = $this->user['id'];
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND "SKP"."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_KEBERATAN.' AND   SKP_PENILAIAN."nrk_atasan_pejabat"=\''.$nrk.'\' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    ...
    public function get_skp_join_hasilperilaku($start = 0, $limit = 10, $str_order= "", $str_where = ""){
        $query = 'select SKP_PENILAIAN."penilaian_approvement",skp.* from '.$this->table.' left JOIN SKP_PENILAIAN on SKP."id"=SKP_PENILAIAN."skp_id" '.$str_where.' AND SKP."status_approvement">='.SKP_DITERIMA.' AND  SKP_PENILAIAN."penilaian_approvement">= '.PENILAIAN_DIKIRIM.' '.$str_order.' OFFSET '.$start.' ROWS FETCH NEXT '.$limit.' ROWS ONLY';        
        return $this->db->query($query)->result();
    }
    //
    ...
    public function get_version($id = ""){
            $query = 'select "version","status" from "'.$this->tblskpdetail.'" where "skp_id"=\''.$id.'\' and "status"=\'0\' group by "version","status" order by "version"';
            return $this->db->query($query)->result();   
    }


    public function get_data_skp_by_nrk($arr_fields = array(), $arr_value = array(), $table_name = ""){    
            foreach ($arr_fields as $key => $value) {
                $this->db->or_where($value , $arr_value[$key]);
            }                                   
    ...
    public function get_by_condition($id="", $tablename = "", $params = array()){    
    ...
    public function delete($id= "", $nrk= ""){
            ...
            $this->sql_update($arr_column, $arr_condition, $this->tblskpdetail);                       
            ...
            //     "status" => 1
            // );  
            // return $this->sql_update($arr_column, $arr_condition, $this->tblskptarget);                       
        }

    public function sql_insert($tbl = "", $arrFields = array(), $arrValues = array()){
            $sql = "insert into \"".$tbl."\"";
            $sql = $sql."(\"".join("\",\"",$arrFields)."\")";            
            $sql = $sql." VALUES ('".join("','",$arrValues)."')";    
            $sql = str_replace("'CURRENT_TIMESTAMP'","CURRENT_TIMESTAMP",$sql);        
            $this->db->query($sql);
            return $sql;
        }

        public function sql_insert2($tbl = "", $arrFields = array()){            
            $sql = "insert into \"".$tbl."\"";
            $fields = "";
            $values = "";            
        ...
        public function sql_update($arr_fields = array(), $arr_condition= array(), $table_name = ""){
            $sql = "update \"".$table_name."\" set ";
            foreach ($arr_fields as $key => $value) {
                if ($value=='CURRENT_TIMESTAMP'){                    
                    $sql = $sql."\"".$key."\"=".$value.",";
                }else if (strpos($value, 'TO_DATE') !== false) {                     
                    $sql = $sql."\"".$key."\"=".$value.",";                    
                }else{
                    $sql = $sql."\"".$key."\"='".$value."',";
                }                
         ...
         // added by pemda, 18-4-2019
        private function _get_datatables_query($approved_waiting= ""){
        //echo "get _get_datatables_query";die;
        $nrk = $this->session->userdata["logged_in"]["nrk"];
        $this->db->from($this->table);
        //$this->db->where("approved_waiting", $nrk);   
        ...
        if ($approved_waiting!=""){
           // $this->db->where("approved_waiting", $approved_waiting);   
            $this->db->where("nrk", $approved_waiting);   
        }
    ...
    private function _get_datatables_query2($nrk= ""){
        //echo "get _get_datatables_query";die;
        $this->db->from($this->table);
        if ($nrk!="")
            $this->db->where("atasan", $nrk);   
        $this->db->where("deleted_at", null);   
        $this->db->where("status_approvement >=", STATUS_INPUT_REALISASI); 
        //$this->db->or_where("approved_by", $nrk); 
        $i = 0;
    
        foreach ($this->column_search as $item) // loop column 
        {
            //var_dump($item);die;
            //echo strtoupper($_POST['search']['value']);die;
            if($_POST['search']['value']) // if datatable send POST for search
            {
    ...
    function get_datatables1($approved_waiting = "")
    {
        $this->_get_datatables_query($approved_waiting);
        if($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();     
    ...
    function get_datatables2($nrk = "")
    {
        $this->_get_datatables_query2($nrk);
        if($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();     
    ...
    public function count_all2()
    {
        ...
        $this->db->where("atasan", $nrk);   
        $this->db->where("deleted_at", null);   
        $this->db->where("status_approvement", 4);   
       // $this->db->or_where("approved_by", $nrk);
        //var_dump($this->db->count_all_results());die;
        return $this->db->count_all_results();
    ...
    function update_realisasi($id= "", $value = array()){   
        // print_r($value);die();
            $arr_column = array("ak_realisasi" => $value[0], 
                                "qty_realisasi"=> $value[1], 
                                "output_realisasi" => $value[2],
                                "quality_realisasi" => $value[3], 
                                "waktu_realisasi" => $value[4],
                                "biaya_realisasi" => $value[5]
                                );            
            $arr_condition = array($this->skpdetail => $id);
            $this->sql_update($arr_column, $arr_condition, $this->tblskpdetail);
    }

    function update_realisasi_approve($id= "", $nrk="", $nrk_atasan=""){    

            $arr_column = array("approved_by" => $nrk,
                                "approved_waiting" => $nrk_atasan,
                                 "status_approvement" => '6'
                                );            
            $arr_condition = array($this->skp => $id);
            $this->sql_update($arr_column, $arr_condition, $this->tblskp);
            
    }

    function update_realisasi_approve_usulan($id= "", $nrk="", $nrk_atasan=""){    

            $arr_column = array("approved_by" => $nrk,
                                "approved_waiting" => $nrk_atasan,
                                "status_approvement" => '5'
                                );            
            $arr_condition = array($this->skp => $id);
            //echo $this->db->last_query();die;
            $this->sql_update($arr_column, $arr_condition, $this->tblskp);
    ...
    function update_realisasi_tolak($id= "", $nrk="", $nrk_atasan=""){    
    ...
    function create_table_log($tableName = ""){
        if ($tableName!=""){
            $sql = "SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH FROM ALL_TAB_COLUMNS WHERE TABLE_NAME='".$tableName."'";            
            // echo $sql;echo "<br>";
            $result = $this->db->query($sql)->result();
            $sql_drop = "DROP TABLE MLOG\$_".$tableName;
            // echo $sql_drop;echo "<br>";
            $this->db->query($sql_drop);
            $sql_create = "CREATE TABLE MLOG\$_".$tableName."(id VARCHAR2(50),";            
            foreach ($result as $key => $value) {
                $colname = strtolower($value->COLUMN_NAME);
                $datatype = $value->DATA_TYPE;
                if (strtoupper($value->COLUMN_NAME) == "ID"){
                    $colname =  $tableName."_id";;
                }
                if (strtoupper($datatype) == "VARCHAR2"){
                    $datatype.= "(".$value->DATA_LENGTH.")";
                }
                $sql_create .= $colname." ".$datatype.", ";             
            }
            $sql_create = rtrim($sql_create,', ');
            $sql_create .= ")";              
            // echo $sql_create;echo "<br>";

            $result = $this->db->query($sql_create);
            return $result;
        }        
    }

    function insert_log($table_name = "", $id = "", $status = ""){
        if ($table_name != ""){
            $sql = 'select * from "'.$table_name.'" where "id"=\''.$id.'\'';
            $result = $this->db->query($sql)->result();
            $result = (array)$result[0];                        
            $result[$table_name."_ID"] = $result["id"];
            $data = array();
            $result["id"] = $this->id();
            foreach ($result as $key => $value) {
            ...
            $result = (object)$data;               
            $this->sql_insert2("MLOG\$_".$table_name, $result);                     
        }
