<?php 
 ...
 class M_admin extends CI_Model {  
    ...
    {
        $sql = "SELECT \"user_group_id\", UPPER(\"nama_group\") \"nama_group\" 
                FROM \"master_user_group\" 
                WHERE \"status_aktif\" = 'Y'
                ORDER BY \"nama_group\" ASC ";
        // $sql = "SELECT USER_GROUP_ID, UPPER(NAMA_GROUP) AS NAMA_GROUP 
        //         FROM MASTER_USER_GROUP 
        //         WHERE STATUS_AKTIF = 'Y'
        //         ORDER BY NAMA_GROUP ASC ";
    ...
    {        
        ...
        $sql = "SELECT a.\"id_menu\", a.\"nama_menu\", b.\"act_view\", b.\"act_insert\", b.\"act_update\", b.\"act_resetpass\", b.\"act_delete\"
                FROM \"menu_master\" a 
    ...
    function insertMenuAccess($data){
        ...
        $resetpass = $this->input->post('actresetpass');
        ...
        if($queryDel){
            ...
            for ($i=0; $i < count($idmenu); $i++) { 
                ...
                $valResPas = isset($resetpass[$idmenu[$i]]) ? $resetpass[$idmenu[$i]] : 'N';
                $valDel = isset($delete[$idmenu[$i]]) ? $delete[$idmenu[$i]] : 'N';
                
                $sql = "INSERT INTO \"menu_access_user\"(\"menu_access_id\", \"menu_id\", \"user_group_id\", \"act_view\", \"act_insert\", \"act_update\", \"act_resetpass\", \"act_delete\", \"input_by\", \"input_date\") 
                        VALUES ('".$id."', '".$menu."', '".$usergroup."', '".$valVw."', '".$valIns."', '".$valUpd."', '".$valResPas."', '".$valDel."', '".$input_by."', TO_DATE('".$this->today."', 'YYYY-MM-DD HH:MI:SS'))";
    ...
    function G_group($nama_group)
    {
        $sql ="SELECT \"nama_group\"
                FROM \"master_user_group\"
                WHERE \"nama_group\" = \"LOWER\"('".$nama_group."') ";
        return $this->db->query($sql)->num_rows();
    }

    function ins_group($nama_group,$status)
    {
        
        $insert = "INSERT INTO \"master_user_group\"(\"user_group_id\",\"nama_group\",\"status_aktif\",\"update_date\")
                    VALUES((SELECT \"MAX\"(\"user_group_id\")+1
                    FROM \"master_user_group\"),\"LOWER\"('".$nama_group."'),'".$status."',SYSDATE) ";

        return $this->db->query($insert);
    }

    function get_group($user_group_id)
    {
        $cek = "SELECT *
                FROM \"master_user_group\"
                WHERE \"user_group_id\" = '".$user_group_id."' ";
        return $this->db->query($cek)->row();
    }

    function edit_group($user_group_id,$status)
    {
        
        $update = "UPDATE \"master_user_group\" SET \"status_aktif\" = '".$status."',\"update_date\" =SYSDATE 
                    WHERE \"user_group_id\" = '".$user_group_id."' ";
