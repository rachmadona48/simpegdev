<?php
 class V_ptt extends CI_Model {
    ...
    public function get_Kolok(){
        $q="SELECT KOLOK,NALOKL
            FROM  pers_lokasi_tbl 
            WHERE (NALOKL != '.' AND AKTIF='1') 
            GROUP BY KOLOK,NALOKL
            ORDER BY nalokl";
    ...
    public function get_lit_spmu(){
        $sql="SELECT KODE_SPM,NAMA,KLOGAD_INDUK,TAHUN from PERS_TABEL_SPMU";
    ...
    public function get_data_ptt(){
        $hak_akses = $this->infopegawai->hakAksesModul('28809',$this->user['user_group']);
        $requestData = $this->input->post(); 
        $pttsrc= $requestData['pttsrc'];
        $koloksrc= $requestData['koloksrc'];
        // echo $koloksrc;exit();

        $where = '';
        if($pttsrc != '' && $koloksrc != ''){
            $where = "WHERE A.KLOG = '".$koloksrc."'  AND (A.NPTT = '".$pttsrc."' or LOWER(A.NAMA) LIKE '%".$pttsrc."%') ";
        }else if($pttsrc != '' && $koloksrc == '') {
            $where = "WHERE (A.NPTT = '".$pttsrc."' or LOWER(A.NAMA) LIKE '%".$pttsrc."%') ";
        }else if($pttsrc == '' && $koloksrc != '') {
            $where = "WHERE A.KLOG = '".$koloksrc."' ";
        };
        ...
        $sql = "SELECT * FROM (
                    SELECT ROWNUM AS RN,A.NORUT,A.NPTT,A.NAMA,A.TGLLAHIR,A.KODEL,A.JABAT,A.KLOG,A.SPMU,A.GTI,A.TTI,A.STATUS,
                        B.NAMA AS SPMU_NAMA,
                        C.NALOKL AS LOKASI
                    FROM
                        PERS_MASTER_PTT_AKTIF A
                    LEFT JOIN PERS_TABEL_SPMU B ON A .SPMU = B.KODE_SPM
                    LEFT JOIN PERS_LOKASI_TBL C ON A .KLOG = C.KOLOK
                    ".$where."
                )A";
        // echo $sql;exit();
        ...
        if( !empty($requestData['search']['value']) ){   //kode nptt
            $sql.=" AND lower(NPTT) LIKE lower('%".$requestData['search']['value']."%') ";
            $sql.=" OR lower(NAMA) LIKE lower('%".$requestData['search']['value']."%') ";
            $sql.=" OR lower(JABAT) LIKE lower('%".$requestData['search']['value']."%') ";
            $sql.=" OR lower(SPMU_NAMA) LIKE lower('%".$requestData['search']['value']."%') ";
            $sql.=" OR lower(LOKASI) LIKE lower('%".$requestData['search']['value']."%') ";
            // echo $sql;
        }
        ...
        $sql.=" ORDER BY \"".$columns[0]."\"   ASC ";  // adding length
        ...
        foreach($query->result() as $row){
            ...
            $nestedData[] = $row->SPMU_NAMA;
            $nestedData[] = $row->LOKASI;
            $nestedData[] = $this->format_uang->rp_format($row->GTI);
            // $nestedData[] = $this->format_uang->rp_format($row->TTI);
            
            if ($hak_akses->act_update == 'Y'){
                $nestedData[] = 
                "<div class='col-sm-2' align='center'>
                    <form method='post' action='".site_url('master_ptt/doedit')."' >
                        <input type='hidden' name='nptt' id='nptt' value='".$row->NPTT."'>
                        <button class='btn btn-outline btn-xs btn-warning' data-toggle='tool-tip' data-placement='bottom' title='edit PTT' type='submit' pull-right><i class='fa fa-pencil-square'></i></button>
                    </form>
                </div>";  
            }else{
                $nestedData[] = "";
    ...
    function get_data($nptt)
     ...
     function get_list_spmu($spmu=""){

        // echo $spmu;exit();
         $sql = "SELECT KODE_SPM,NAMA,TAHUN FROM PERS_TABEL_SPMU";
         ...
         foreach($id->result() as $row){
             if($spmu == $row->KODE_SPM){
                 $option .= "<option selected value='".$row->KODE_SPM."'>".$row->NAMA."</option>";
             }else{
     ...
     function get_list_klog($klog=""){

        // echo $spmu;exit();
         $sql = "SELECT KOLOK,NALOKL,NALOKS FROM PERS_LOKASI_TBL";
         ...
         foreach($id->result() as $row){
             if($klog == $row->KOLOK){
                 $option .= "<option selected value='".$row->KOLOK."'>".$row->NALOKL."</option>";
             }else{
     ...
     public function update_data($nptt,$nama,$jabat,$spmu,$klog,$gti,$kodel,$status){
        $sql = "UPDATE PERS_MASTER_PTT_AKTIF SET
                        nama = '".$nama."',
                        jabat = '".$jabat."',
                        spmu='".$spmu."',
                        klog='".$klog."',
...
ORDER BY B.NAMA ASC";
    ...
    function getKolokSKP(){
     ...
     //            FROM PERS_KOWIL_TBL
     //            WHERE KOWIL LIKE '".substr($prop, 0, 3)."%'";
     //     $id = $this->db->query($sql);
     ...
     // function getListProp($prop=""){
     //     $sql = "SELECT PROP, KETERANGAN FROM PERS_PROP_RPT ORDER BY PROP";
     //     $id = $this->db->query($sql);
