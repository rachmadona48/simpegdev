<?php 
 ...
 class Mhome extends CI_Model {
    ...
    {
        $sql="SELECT A.NRK,A.JENCUTI,A.TGAKHIR,A.TG_UPD,A.FILE_UPLOAD,A.STATUS_FILE_UPLOAD,TO_CHAR(A.tmt, 'DD-MM-YYYY')TMT, A.NOSK, TO_CHAR(A.TGSK,'DD-MM-YYYY')TGSK,B.KETERANGAN
                FROM pers_cuti_hist A
                INNER JOIN PERS_JENCUTI_RPT B ON A.JENCUTI=B.JENCUTI
    ...
    {             
        $sql = "SELECT A.NRK, A.TGIJAZAH,A.FILE_UPLOAD,A.STATUS_FILE_UPLOAD,A.KODIK, A.NASEK,A.KOTSEK, A.JENDIK,A.UNIVER,B.NAUNIVER,A.NOIJAZAH
    ...
    {             
        $sql = "SELECT NRK, to_char(TGIJAZAH, 'YYYY-MM-DD') TGIJAZAH,FILE_UPLOAD,KODIK,STATUS_FILE_UPLOAD, NASEK,KOTSEK, JENDIK,NOIJAZAH
    ...
    {
        $sql ="SELECT a.NRK, a.KDHARGA,a.STATUS_FILE_UPLOAD,a.FILE_UPLOAD, b.NAHARGA, to_char(a.TGSK, 'YYYY-MM-DD') TGSK, a.NOSK, a.ASAL_HRG
    ...
    {
        $sql ="SELECT A.NRK,A.TMT,A.JENRUB,A.FILE_UPLOAD,A.STATUS_FILE_UPLOAD, A.KOPANG,A.NOSK,TO_CHAR(A.TGSK,'DD-MM-YYYY')TGSK,A.GAPOK,A.TTMASKER,A.BBMASKER,A.KOLOK,B.GOL, B.NAPANG
                FROM PERS_RB_GAPOK_HIST A
                LEFT JOIN PERS_PANGKAT_TBL B ON A.KOPANG=B.KOPANG 
    ...
    function getUserHubkel($nrk)
    {
        ...
        $sql="SELECT a.NRK, a.NIK, a.STATUS_FILE_UPLOAD,a.FILE_UPLOAD, a.HUBKEL, b.NAHUBKEL, a.NAMA, a.TEMHIR, to_char(a.TALHIR, 'YYYY-MM-DD') TALHIR, to_char(a.TGNIKAH, 'YYYY-MM-DD') TGNIKAH, a.TEMNIKAH, c.KETERANGAN TUNJANGAN, d.KETERANGAN KERJAAN, a.JENKEL, to_char(a.MATI, 'YYYY-MM-DD') MATI, a.UANGDUKA
                FROM PERS_KELUARGA a
                LEFT JOIN PERS_HUBKEL_TBL b ON a.HUBKEL = b.HUBKEL
                LEFT JOIN PERS_STATTUN_RPT c ON a.STATTUN = c.STATTUN
                LEFT JOIN PERS_KDKERJA_RPT d ON a.KDKERJA = d.KDKERJA
    ...
    {
        $sql = "SELECT KO.NRK,to_char(KO.TMT, 'YYYY-MM-DD') TMT, KO.NOSK,KO.FILE_UPLOAD,KO.STATUS_FILE_UPLOAD, to_char(KO.TGSK, 'YYYY-MM-DD') TGSK, KO.KOPANG, TB.GOL, TB.NAPANG, KO.KOLOK, LO.NALOKL, KO.GAPOK
    ...
    function getUserJabatanS($nrk)
    {
        ...
        $sql = "SELECT JH.NRK, JH.TMT,JH.FILE_UPLOAD,JH.STATUS_FILE_UPLOAD, JH.KOLOK, JH.TGSK, JH.NOSK, JH.KOJAB, KO.NAJABL,PG.NAPANG,PG.GOL, LO.NALOKL,JH.ESELON
                FROM PERS_JABATAN_HIST JH
                LEFT JOIN PERS_KOJAB_TBL KO ON JH.KOLOK = KO.KOLOK AND JH.KOJAB = KO.KOJAB
                LEFT JOIN PERS_PANGKAT_TBL PG ON JH.KOPANG = PG.KOPANG
                LEFT JOIN PERS_LOKASI_TBL LO ON JH.KOLOK = LO.KOLOK
    ...
    function getUserJabatanF($nrk)
    {
        ...
        $sql = "SELECT JH.NRK, JH.TMT,JH.TGSK,JH.FILE_UPLOAD,JH.STATUS_FILE_UPLOAD,JH.KOLOK, JH.NOSK,JH.KOJAB, KO.NAJABL,PG.NAPANG,PG.GOL,LO.NALOKL
                FROM PERS_JABATANF_HIST JH
                LEFT JOIN PERS_KOJABF_TBL KO ON JH.KOJAB = KO.KOJAB
                LEFT JOIN PERS_PANGKAT_TBL PG ON JH.KOPANG = PG.KOPANG
                LEFT JOIN PERS_LOKASI_TBL LO ON JH.KOLOK = LO.KOLOK
    ...
    {
        $sql ="SELECT NRK,TAHUN,PELAYANAN,FILE_UPLOAD,STATUS_FILE_UPLOAD,INTEGRITAS,KOMITMEN,DISIPLIN,KERJASAMA,KEPEMIMPINAN,JUMLAH,RATA2,NILAI_SKP,NILAI_PERILAKU,NILAI_PRESTASI,F_GET_NAMA(NRK_PEJABAT_PENILAI)NAMA_PEJABAT_PENILAI, F_GET_NAMA(NRK_ATASAN_PEJABAT_PENILAI)NAMA_ATASAN_PEJABAT_PENILAI,INPUT_SKP,F_KET_SKP(NILAI_PRESTASI)KET_PRESTASI
                FROM PERS_skp A
